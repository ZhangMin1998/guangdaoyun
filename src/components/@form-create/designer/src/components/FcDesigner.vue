    ...
    <ElMain>
      <ElContainer style="height: 100%;">
        ...
        <ElDialog v-model="preview.state" width="800px" append-to-body>
          <ViewForm :rule="preview.rule" :option="preview.option" v-if="preview.state" @update:modelValue="viewChange"></ViewForm>
        </ElDialog>
      </ElContainer>
    </ElMain>
  </ElContainer>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const methods = {
      ...
      viewChange (formData) {
        console.log('11111111111111111111111111111', formData, data.preview, methods.getJson())
        vm.emit('valueChange', formData)
      },
      updatePreviewRuleValue (field, value) {
        const filedIndex = data.preview.rule.findIndex(item => item.field === field)
        filedIndex > -1 && (data.preview.rule[filedIndex].value = value)
        console.log('222233333', filedIndex, field, value, data.preview.rule)
      },
      ...
      toolActive(rule) {
        ...
        console.log('111', data.activeRule)
    ...
    provide('getJson', methods.getJson)
    return {
